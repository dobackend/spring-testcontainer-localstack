...
public class UserRepository {
    ...
    private static final String UPSERT_QUERY = "INSERT INTO USERS (USER_ID, NAME, SEQUENCE_NUMBER) VALUES (?, ?, ?) " +
            "ON CONFLICT (USER_ID) DO UPDATE SET " +
            "NAME = EXCLUDED.NAME, " +
            "SEQUENCE_NUMBER = EXCLUDED.SEQUENCE_NUMBER " +
            "WHERE USERS.SEQUENCE_NUMBER <= EXCLUDED.SEQUENCE_NUMBER";
...
//        return jdbcTemplate.query(FIND_ALL_QUERY, new UserMapper());
    ...
    public Boolean upsert(User user, Long sequenceNumber) {
        return jdbcTemplate.update(UPSERT_QUERY,
                user.userId(),
                user.name(),
                sequenceNumber
        ) > 0;
